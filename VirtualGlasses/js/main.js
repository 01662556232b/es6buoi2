...
let renderthukinh = (dataGlasses) => {
    let content = "";
    dataGlasses.map((gl) => { 
        content +=`
            <div class="col-4">
                  <img onclick="showthukinh('${gl.id}')" id='${gl.id}' class="img-fluid" src='${gl.src}'>
            </div>
        `
     });
     document.getElementById("vglassesList").innerHTML = content;
}
renderthukinh(dataGlasses);
